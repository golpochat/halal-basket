...
export function DriverHistoryPage() {
  useDashboardTitle('History');
  const { session } = useAuth();
  const [orders, setOrders] = useState<Fulfillment[]>([]);
  const [page, setPage] = useState(1);

  async function refresh() {
    const list = await api<Fulfillment[]>('/driver/orders/history', {
  ...
  useEffect(() => {
    refresh().catch((e) => toastError(e, 'Could not load delivery history'));
  }, [session]);
    ...
    <div>
      <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
        <p className="text-sm text-[var(--hb-ink)]/55">
          {orders.length} past deliveries · {PAGE_SIZE} per page
        </p>
        <IconButton
          ...
          onClick={() =>
            refresh().catch((e) =>
              toastError(e, 'Could not refresh delivery history'),
            )
          }
        >
